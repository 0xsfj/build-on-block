...
import { Divider, Flex, HStack, IconButton, Spacer, Stack, TabIndicator, TabList, Tabs, useColorModeValue as mode, useDisclosure } from '@chakra-ui/react';
...
export const Template: React.FC = (props) => {
  ...
  return (
    <Flex py={4} px={{ base: 4, md: 6, lg: 8 }} bg={mode('white', 'gray.800')} boxShadow="none" borderBottomWidth={mode('0', '1px')}>
      ...
      <HStack display={{ base: 'none', md: 'flex' }} marginStart={4}>
        <Tabs colorScheme="blue" variant="unstyled" isFitted>
          ...
          <TabIndicator mt="13px" height={1} borderTopRadius="md" bg={mode('blue.500', 'blue.200')} />
        </Tabs>
      </HStack>
      <Spacer />
      ...
      <MobileNavContent isOpen={mobileNav.isOpen} onClose={mobileNav.onClose}>
        <Stack spacing={5}>
          ...
          <Tabs orientation="vertical" variant="unstyled">
            <TabList>{children.find((child) => child.type === Links)?.props.children}</TabList>
            <TabIndicator marginStart="-3" width={1} borderTopRadius={{ base: 'none', md: 'md' }} bg={mode('blue.500', 'blue.200')} />
          </Tabs>
          <Divider />
